...
def convertir_fechas(df):
    """Convierte fechas a datetime y extrae componentes útiles."""
...
def limpiar_numero(valor):
    """Convierte strings como '1,234.56' o '1.234,56' a float."""
    if isinstance(valor, (int, float)):
        return valor
    if isinstance(valor, str):
        # Eliminar separadores de miles (punto o coma) y normalizar decimales
...
def obtener_token_square():
    ...
    # Primero intentar desde st.secrets (Streamlit Cloud)
    try:
    ...
    # Si no, intentar desde variables de entorno (local)
    ...
    try:
        ...
        with st.spinner("🔄 Obteniendo datos de Square..."):
            result = client.orders.list_orders(
                ...
                end_time=end_str,
                limit=200  # Límite de órdenes por petición (máximo 200)
            )
            
            orders = result.body.get('orders', [])
            
            # Si no hay órdenes, intentar obtener catálogo para generar datos de prueba
            if not orders:
                st.warning("⚠️ No se encontraron órdenes en el rango de fechas seleccionado.")
                # Generar datos de demostración con productos del catálogo
                return generar_datos_demo(fecha_inicio, fecha_fin)
            
            # Transformar órdenes a DataFrame
            datos = []
            for order in orders:
                # Datos básicos
        ...
        error_msg = f"Error de API: {e.body if hasattr(e, 'body') else str(e)}"
...
def generar_datos_demo(fecha_inicio, fecha_fin):
    """
    Genera datos de demostración cuando no se pueden obtener de Square.
    Útil para probar el dashboard sin conexión.
    """
    ...
    # Generar un registro por hora para cada día en el rango
    ...
    for dia in range(dias):
        ...
        for hora in range(10, 21):  # De 10:00 a 20:00
            ...
            # Seleccionar 2-4 vendedores por turno
            ...
            for v in vendedores_turno:
                # Asignar AOV y UPT según perfil
                if "Asesor" in v:
                    aov_base = 48
                    u_base = 2.9
                elif "Senior" in v:
...
# ------------------------------------------------------------
# FUNCIÓN PARA GENERAR DICTAMEN EN TEXTO SIMPLE
# ------------------------------------------------------------
def generar_dictamen_simple(vendedores):
    """Genera un dictamen en formato texto simple para mostrar en Streamlit."""
    if not vendedores:
        return "No hay datos de vendedores disponibles."
...
def generar_pdf_simple(resultado, fecha_inicio, fecha_fin, cliente="Demo"):
    """Genera un PDF básico con los KPIs principales."""
    try:
...
with st.sidebar:
    ...
    # Botón de carga de demo (datos sintéticos)
